...
findAllInputs.forEach((input) => {
  this
  if (input.value.trim().length === 0 && input.hasAttribute ('required')) {errorMessage.textContent = "*заполните, пожалуйста, все поля";
...
function validatecheckbox() {  // Additional validation for agreement field
  let isChecked = false;
